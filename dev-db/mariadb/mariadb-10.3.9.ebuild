...
COMMON_DEPEND="
	...
	!client-libs? ( dev-db/mariadb-connector-c[${MULTILIB_USEDEP},static-libs?] dev-db/mysql-connector-c[${MULTILIB_USEDEP},static-libs?] )
"
DEPEND="virtual/yacc
...
src_prepare() {
	epatch "${FILESDIR}"/${PN}-10.3.9-fix-mysql-install-db-path.patch
	epatch "${FILESDIR}"/${PN}-10.3.9-fix-pthread-detach.patch
	epatch "${FILESDIR}"/${PN}-10.3.9-fix-ucontext-check.patch
	epatch "${FILESDIR}"/${PN}-10.3.9-pcre.cmake.patch
	epatch "${FILESDIR}"/${PN}-10.3.9-ppc-remove-glibc-dep.patch
...
src_configure(){
	...
	mycmakeargs=(
		...
		-DSUFFIX_INSTALL_DIR=""
		-DWITH_UNITTEST=OFF
		-DWITHOUT_CLIENTLIBS=$(usex client-libs NO YES)
