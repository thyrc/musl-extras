...
IUSE="clippy cpu_flags_x86_sse2 debug doc libressl rls rustfmt system-llvm system-rust wasm ${ALL_LLVM_TARGETS[*]}"
...
PATCHES=( "${FILESDIR}"/1.38.0-fix-custom-libdir.patch
		  "${FILESDIR}"/1.38.0-fix-multiple-llvm-rebuilds.patch
		  "${FILESDIR}"/1.36.0-libressl.patch
		  "${FILESDIR}"/1.36.0-libressl3.patch
		  "${FILESDIR}"/${PN}-1.36.0-Require-static-native-libraries-when-linking-static.patch
          "${FILESDIR}"/${PN}-1.38.0-Remove-nostdlib-and-musl_root.patch
          "${FILESDIR}"/${PN}-1.33.0-Switch-musl-targets-to-link-dynamically-by-default.patch
          "${FILESDIR}"/${PN}-1.38.0-Fix-vendor-linux_musl_base-checksum.patch
          ...
          "${FILESDIR}"/${PN}-1.38.0-Fix-vendor-liblibc-checksum.patch
          "${FILESDIR}"/${PN}-1.34.0-Avoid_LLVM_name_conflicts.patch
          "${FILESDIR}"/${PN}-1.36.0-Fix-codegen-llvm-link-typo.patch
          "${FILESDIR}"/${PN}-1.34.0-llvm-secureplt.patch
)
