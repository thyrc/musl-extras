...
PATCHES=(
	...
	"${FILESDIR}"/${PN}-1.45.0-Remove-nostdlib-and-musl_root.patch
	"${FILESDIR}"/${PN}-1.45.0-Remove_crt_fallback_functions_for_musl.patch
	"${FILESDIR}"/${PN}-1.33.0-Switch-musl-targets-to-link-dynamically-by-default.patch
	"${FILESDIR}"/${PN}-1.45.0-Fix-vendor-linux_musl_base-checksum.patch
	"${FILESDIR}"/${PN}-1.33.0-liblibc.patch
	"${FILESDIR}"/${PN}-1.44.0-Fix-vendor-liblibc-checksum.patch
